...
1. **구현**: 기능 코드 작성
2. **테스트**: Playwright MCP로 즉시 테스트 수행
3. **검증**: 테스트 결과 확인 및 스크린샷 기록
4. **수정**: 테스트 실패 시 코드 수정 후 재테스트
5. **완료**: 모든 테스트 통과 후에만 다음 단계 진행

**⚠️ 테스트 통과 전 다음 단계 진행 금지**
...
API 연동 및 비즈니스 로직 구현 시 **Playwright MCP**를 활용한 테스트는 필수입니다.

#### **테스트 원칙**

1. **구현 → 테스트 → 검증 사이클**: 모든 기능 구현 후 즉시 테스트 수행
2. **테스트 실패 시 구현 수정**: 테스트 통과 전까지 다음 단계 진행 금지
3. **E2E 테스트 우선**: 사용자 관점의 전체 플로우 테스트 우선 수행
...
| `browser_snapshot` | 현재 페이지 상태 캡처 및 요소 확인 |
| `browser_click` | 버튼/링크 클릭 테스트 |
| `browser_fill_form` | 폼 입력 테스트 |
| `browser_take_screenshot` | 시각적 검증을 위한 스크린샷 |

#### **테스트 시나리오 작성 예시**

**API 연동 테스트:**
1. 페이지 로드 후 API 응답 데이터 표시 확인
2. 로딩 상태 → 데이터 표시 상태 전환 확인
3. API 에러 시 에러 메시지 표시 확인

**비즈니스 로직 테스트:**
1. 사용자 입력 → 처리 → 결과 표시 플로우
2. 유효성 검사 동작 확인
3. 엣지 케이스 (빈 값, 잘못된 형식 등) 처리 확인

#### **테스트 체크리스트 템플릿**

Task 파일에 다음 형식의 테스트 체크리스트 필수 포함:
